...
test.describe("Product filtering and checkout with applying discount", () => {
  ...
  test.beforeEach("Visit the website", async ({ page, context }) => {
    ...
    await test.step("Add cookies for hiding cookie consent", async () => {
      await cookies.handleCookies();
    });

    await test.step(("Add loadhandler for waiting for loader to hide"), async () => {
      loader.waitForLoaders()
    })
  ...
  products.forEach((product: Product) => {
    test(`Navigate menu items ${product.mainCategory} > ${product.category}, apply filters on product listing page and add item to the cart`, async () => {
      await test.step(`Navigate to the product listing page: ${product.mainCategory}`, async () => {
        await pageNavigation.clickMainMenuItem(product.mainCategory);
      });

      await test.step(`Select the category: ${product.category}`, async () => {
        ...
        for (const filter of product.filters) {
          if (filter.isProductVariant) {
            await test.step(`Select product variant if applicable`, async () => {
              await productDetailsPage.selectProductVariant(filter.name, filter.value);
            });
          }
        }
      })
      ...
      await test.step(`Verify that the order total cost is correct`, async () => {
        await paymentsPage.verifyOrderTotal(productPrice, product.quantity, shippingCosts);
      });
    });
  });
});
